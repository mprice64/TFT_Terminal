...
//    delay(1000);
    ...
    EPAPER.clear_sd();                                  // clear sd card data(when use display next time, display is clear)    
    EPAPER.drawNumber( count++, 5, 5 );

    EPAPER.drawString("Who are you that flies", 5, 30);
    EPAPER.drawString("so good? Are you", 5, 45);
    EPAPER.drawString("insane?", 5, 60);

    EPAPER.drawString(" - Mprice was here", 45, 75);
